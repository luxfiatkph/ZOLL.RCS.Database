...
{
    [TestClass]
    public class Asp330TestAspSelfCheckIntegrationTests : DataIntegrationSubItemTestBase<Asp330Test,
        Asp330TestAspSelfCheck, Guid, IAsp330TestRepository, IAsp330TestAspSelfCheckRepository>
    {
        public Asp330TestAspSelfCheckIntegrationTests()
        {
            ItemRepository = UnitOfWork.Asp330Tests;
            SubItemRepository = UnitOfWork.Asp330TestAspSelfChecks;
            Entities = FakerAsp330.Asp330TestFaker(ListCount);
            var ids = Entities.Select(testEntity => testEntity.Asp330TestId).ToList();
            SubEntities = FakerAsp330.Asp330TestAspSelfCheckFaker(ids);
        }

        [TestMethod]
        public void CrudTest()
        {
            CrudTest(nameof(Asp330TestAspSelfCheck.Asp330TestId));
        }
        ...
        {
            // Arrange
            var itemId1 = SubEntities[0].Asp330TestId;
            var itemId2 = SubEntities[2].Asp330TestId;
