...
{
    [TestClass]
    public class Asp330TestDatetimeCheckIntegrationTest : DataIntegrationSubItemTestBase<Asp330Test,
        Asp330TestDatetimeCheck, Guid, IAsp330TestRepository, IAsp330TestDatetimeCheckRepository>
    {
        public Asp330TestDatetimeCheckIntegrationTest()
        {
            ItemRepository = UnitOfWork.Asp330Tests;
            SubItemRepository = UnitOfWork.Asp330TestDatetimeChecks;
            Entities = FakerAsp330.Asp330TestFaker(ListCount);
            var ids = Entities.Select(testEntity => testEntity.Asp330TestId).ToList();
            SubEntities = FakerAsp330.Asp330TestDatetimeCheckFaker(ids);
        }
