...
{
    [TestClass]
    public class Asp330TestLedCheckIntegrationTests : DataIntegrationSubItemTestBase<Asp330Test,
        Asp330TestLedCheck, Guid, IAsp330TestRepository, IAsp330TestLedCheckRepository>
    {
        public Asp330TestLedCheckIntegrationTests()
        {
            ItemRepository = UnitOfWork.Asp330Tests;
            SubItemRepository = UnitOfWork.Asp330TestLedChecks;
            Entities = FakerAsp330.Asp330TestFaker(ListCount);
            var ids = Entities.Select(testEntity => testEntity.Asp330TestId).ToList();
            SubEntities = FakerAsp330.Asp330TestLedCheckFaker(ids);
        }
