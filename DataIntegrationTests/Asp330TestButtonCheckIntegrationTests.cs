...
{
    [TestClass]
    public class Asp330TestButtonCheckIntegrationTests : DataIntegrationSubItemTestBase<Asp330Test,
        Asp330TestButtonCheck, Guid, IAsp330TestRepository, IAsp330TestButtonCheckRepository>
    {
        public Asp330TestButtonCheckIntegrationTests()
        {
            ItemRepository = UnitOfWork.Asp330Tests;
            SubItemRepository = UnitOfWork.Asp330TestButtonChecks;
            Entities = FakerAsp330.Asp330TestFaker(ListCount);
            var ids = Entities.Select(testEntity => testEntity.Asp330TestId).ToList();
            SubEntities = FakerAsp330.Asp330TestButtonCheckFaker(ids);
        }

        [TestMethod]
        public void CrudTest()
        {
            CrudTest(nameof(Asp330TestButtonCheck.Asp330TestId));
        }
